...
class VehicleTest {
	...
	void getset_contamination_class() {
		...
		assertEquals(1, v1.get_contaminationClass());

		// setContClass changes the contamination class
		v1.setContaminationClass(4);
		assertEquals(4, v1.get_contaminationClass());
	...
	void getset_test_speed() {
		...
		assertEquals(0, v1.get_actualSpeed());
		v1.setSpeed(30);
		assertEquals(0, v1.get_actualSpeed());
		...
		v1.moveToNextRoad();
		v1.setSpeed(30);
		assertEquals(30, v1.get_actualSpeed());
	...
	void test_iterinary_is_readonly() {
		...
		// a vehicle
		Vehicle v1 = new Vehicle("v1", 50, 1, Arrays.asList(j1, j2));
		assertThrows(UnsupportedOperationException.class, () -> v1.get_itinerary().add(j3));
	...
	void test_1() {
		...
		assertNull(v1.get_road());

		// the velocity should be 0
		assertEquals(0, v1.get_actualSpeed());

		// the status of v1 should be PENDING
		assertEquals(VehicleStatus.PENDING, v1.get_status());

		// no vehicle on the road yet
		assertEquals(0, r1.get_vehicleList().size());

		// enter the road
		v1.moveToNextRoad();

		// the status of v1 should be TRAVELING
		assertEquals(VehicleStatus.TRAVELING, v1.get_status());

		// the road should be r1
		assertEquals(v1.get_road(), r1);

		// the list of vehicles on r1 should be [v1]
		assertEquals(r1.get_vehicleList().size(), 1);
		assertEquals(r1.get_vehicleList().get(0), v1);

		// move the vehicle in road 'r1'
		r1.advance(0);

		// the last advance should move vehicle 'v1' for 50 units
		assertEquals(50, v1.get_location());
